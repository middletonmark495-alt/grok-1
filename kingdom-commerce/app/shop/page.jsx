...
const DEMO = [
  ...
  { id: 'd6', name: 'Royal Chain', category: 'accessories', price: 450, images: ['https://images.unsplash.com/photo-1599643478518-a784e5dc4c8f?w=600&q=80'], is_new: true },
  ...
  { id: 'd8', name: 'Kingdom Tote', category: 'accessories', price: 120, original_price: 160, images: ['https://images.unsplash.com/photo-1584917865442-de89df76afd3?w=600&q=80'], is_sale: true },
];
      ...
      <div className="container">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32, flexWrap: 'wrap', gap: 16 }}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {categories.map((c) => (
              <button key={c} onClick={() => setActiveCategory(c)} style={{ padding: '8px 20px', fontSize: '0.75rem', letterSpacing: 1.5, textTransform: 'uppercase', fontWeight: 500, background: activeCategory === c ? 'var(--gold)' : 'transparent', color: activeCategory === c ? 'var(--black)' : 'var(--gray)', border: `1px solid ${activeCategory === c ? 'var(--gold)' : '#333'}`, cursor: 'pointer', transition: 'all 0.2s' }}>{c}</button>
            ))}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <span style={{ color: 'var(--gray)', fontSize: '0.85rem' }}>{filtered.length} products</span>
            <div style={{ position: 'relative' }}>
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} style={{ appearance: 'none', background: 'var(--dark-2)', border: '1px solid #333', color: 'var(--white)', padding: '10px 36px 10px 16px', fontSize: '0.8rem', cursor: 'pointer' }}>
        ...
        {loading
          ? <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 32 }}>{Array(8).fill(0).map((_, i) => <div key={i}><div className="skeleton" style={{ aspectRatio: '3/4', marginBottom: 16 }} /></div>)}</div>
          : <motion.div layout style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 32, paddingBottom: 80 }}>
